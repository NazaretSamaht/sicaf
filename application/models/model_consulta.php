...
class Model_consulta extends CI_Model {
    ...
    function datos_solicitud($id_solicitud){
        $sql="select * from consulta_general_solicitudes where id=$id_solicitud";
        //echo $sql;
        $row = (array)$this->db->query($sql)->row();
    }


    function solicitud_documento(){
        $aColumns = array( 'codigo','cedula','nombre_apellido', 'nominas','fecha_solicitud','observacion', 'procedimientos','id' );
         
        /* Indexed column (used for fast and accurate table cardinality) */
        $sIndexColumn = "id";
         
        /* DB table to use */
        $sTable = "  consulta_general_solicitudes";
        ...
        while ( $aRow = pg_fetch_array($rResult, null, PGSQL_ASSOC) )
        {
            ...
            for ( $i=0 ; $i<count($aColumns) ; $i++ )
            {
                if ( $aColumns[$i] == "id" )
                {
                    ...
                    $row[] = "<a href='".  site_url('consulta/documentos')."/{$aRow[ $aColumns[$i] ]}'><b><i class='fa fa-copy fa-lg'></i></b></a>&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href='".  site_url('pdf/acuse_recibo')."/{$aRow[ $aColumns[$i]]}' target='_blank'><b><i class='fa fa-print fa-lg'></i></b></a>";
                }
                else if ( $aColumns[$i] != ' ' )
                {
                    /* General output */
